...
class MyLoader(object):
    ...
    def loadData(self):
        ...
        kg_interactive = pd.DataFrame({'head': self.train['userid'].to_list(), 'relation': [0] * len(self.train),
                                       'tail': self.train['itemid'].to_list()})
        kg_interactive_r = pd.DataFrame({'head': self.train['itemid'].to_list(), 'relation': [1] * len(self.train),
